...
class FirestoreSessionStore {
  ...
  snapshotPathPrefix;
  constructor(opts) {
    this.db = opts?.db ?? new import_firestore.Firestore();
  ...
  async saveSnapshot(snapshotId, mutator, options) {
    return this.db.runTransaction(async (tx) => {
      ...
      if (!sessionId) {
        throw new import_beta.GenkitError({
      ...
      if (existing) {
        if (existing.doc.kind === "checkpoint") {
          ...
          const candidatePatch = (0, import_beta.diff)(parentState, newState);
  ...
  async reconstructFrom(reader, checkpointId, shardCount, segmentPath, targetId, options) {
    ...
    for (const ref of segRefs) {
      ...
      state = (0, import_beta.applyPatch)(state, segDoc.statePatch ?? []);
      targetDoc = segDoc;
    }
