...
__export(tool_exports, {
  ...
  isMultipartTool: () => isMultipartTool,
...
function toToolDefinition(tool2) {
  ...
  if (tool2.__action.key) {
    out.key = tool2.__action.key;
  }
...
function implementTool(a, config, registry) {
  a.respond = (interrupt2, responseData, options) => {
    ...
    responseData = (0, import_schema.parseSchema)(responseData, {
      jsonSchema: config.outputJsonSchema,
      schema: config.outputSchema
    });
    return respondTool(interrupt2, responseData, options);
  };
  a.restart = (interrupt2, resumedMetadata, options) => {
    ...
    return restartTool(interrupt2, resumedMetadata, { replaceInput });
  };
}
function restartTool(interrupt2, resumedMetadata, options) {
  let replaceInput = options?.replaceInput;
  return {
...
function respondTool(interrupt2, responseData, options) {
  return {
...
function isMultipartTool(t) {
  return (0, import_core.isAction)(t) && t.__action?.metadata?.type === "tool.v2";
}
function interrupt(config) {
  ...
  return tool(
    ...
    async (input, { interrupt: interrupt2 }) => {
