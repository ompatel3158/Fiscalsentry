...
function defineCustomAgent(registry, config, fn) {
  ...
  const getSnapshotDataAction = (0, import_core.defineAction)(
    registry,
    {
      ...
      outputSchema: import_session.SessionSnapshotSchema
    },
    async (lookup) => {
      const snap = await resolveSnapshot({ ...lookup, context: (0, import_core.getContext)() });
      if (!snap) {
...
function validateResumeAgainstHistory(resume, history) {
  const findToolRequest = (name, ref) => {
