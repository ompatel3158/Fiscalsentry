...
async function normalizeMiddleware(registry, middlewareList) {
  ...
  for (let i = 0; i < middlewareList.length; i++) {
    ...
    if (typeof middleware === "function" && middleware.instantiate && middleware.plugin) {
      throw new GenkitError({
        status: "INVALID_ARGUMENT",
        message: `Middleware ${middleware.name || "function"} must be called with () when used in 'use' array.`
      });
    }
          ...
          model: async (req, ctx, next) => {
            if (middleware.length === 3) {
              return middleware(
                req,
                ctx,
                async (modifiedReq, opts) => next(modifiedReq || req, opts || ctx)
              );
            } else {
              return middleware(
                req,
                async (modifiedReq) => next(modifiedReq || req, ctx)
              );
            }
          }
        })
    ...
    if (typeof middleware === "object" && middleware !== null && "instantiate" in middleware && typeof middleware.instantiate === "function") {
      const def = middleware;
      const registered = await registry.lookupValue(
        "middleware",
        def.name
      );
      if (!registered) {
    ...
    if (typeof middleware === "object" && middleware !== null && "name" in middleware) {
      const ref = middleware;
      const registered = await registry.lookupValue(
        "middleware",
        ref.name
      );
...
async function generate(registry, options) {
  ...
  resolvedOptions.use = middlewareRefs;
...
function maybeRegisterDynamicMiddlewareTools(registry, middlewares) {
