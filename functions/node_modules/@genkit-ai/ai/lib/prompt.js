...
function promptMetadata(options) {
  const metadata = {
    ...
    prompt: {
      ...
      tools: (options.tools ?? []).map(toolName).filter(Boolean),
      toolChoice: options.toolChoice,
      use: (options.use ?? []).map(toMiddlewareMetadata).filter(Boolean)
    },
    type: "prompt"
  };
  if (options.variant) {
...
function toolName(tool) {
  ...
  if ((0, import_core.isAction)(tool)) {
...
function toMiddlewareMetadata(middleware) {
  if (typeof middleware === "string") {
    return { name: middleware };
  }
  if (typeof middleware === "object" && middleware !== null && middleware.name) {
  ...
  definePromptAsync(
    ...
    (0, import_async.lazy)(async () => {
      ...
      const metadata = {
        ...
        prompt: {
          ...promptMetadata2,
          use: (Array.isArray(promptMetadata2.raw?.["use"]) ? promptMetadata2.raw?.["use"] : []).map(toMiddlewareMetadata).filter(Boolean),
