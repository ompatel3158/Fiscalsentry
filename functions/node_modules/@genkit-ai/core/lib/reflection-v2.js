...
class ReflectionServerV2 {
  ...
  async handleRunAction(request) {
    if (!request.id) return;
    const { key, input, init, context, telemetryLabels, stream, streamInput } = import_reflection_types.ReflectionRunActionParamsSchema.parse(request.params);
    ...
    let inputChannel;
    if (streamInput) {
      inputChannel = new import_async.Channel();
    ...
    try {
      ...
      const errorResponse = {
        code: isAbort ? import_action.StatusCodes.CANCELLED : (0, import_action.statusNameToCode)(err?.status) ?? import_action.StatusCodes.INTERNAL,
  ...
  handleSendInputStreamChunk(request) {
    const { requestId, chunk } = import_reflection_types.ReflectionSendInputStreamChunkParamsSchema.parse(request.params);
  ...
  handleEndInputStream(request) {
    const { requestId } = import_reflection_types.ReflectionEndInputStreamParamsSchema.parse(
      request.params
    );
    const channel = this.inputStreams.get(requestId);
    if (channel) {
